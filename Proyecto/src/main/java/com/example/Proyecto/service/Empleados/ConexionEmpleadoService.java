...
public class ConexionEmpleadoService {
    ...
    public boolean actualizarEmpleado(int id, Map<String, Object> camposActualizar) {
        ...
        if (camposActualizar.containsKey("nombre")) {
            if (!primero) sql.append(", ");
            sql.append("NOMBRE_EMPLEADO = ?");
            parametros.add(camposActualizar.get("nombre"));
            primero = false;
        }

        if (camposActualizar.containsKey("email")) {
            if (!primero) sql.append(", ");
            sql.append("EMAIL_EMPLEADO = ?");
            parametros.add(camposActualizar.get("email"));
            primero = false;
        }

        if (camposActualizar.containsKey("contrasena")) {
            if (!primero) sql.append(", ");
            sql.append("CONTRASENA_EMPLEADO = ?");
            String contrasenaHasheada = passwordEncoder.encode((String) camposActualizar.get("contrasena"));
