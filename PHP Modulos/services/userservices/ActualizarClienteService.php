...
class ActualizarClienteService {
    public function getActualizacionEndpoint(string $tipo, int $id): string {
        return match ($tipo) {
            'cliente' => endpointActualizacion::cliente($id),
            default => throw new InvalidArgumentException("Tipo desconocido: $tipo"),
        };
    }
    
    public function actualizarCliente(int $id_cliente, array $datosCliente): array {
        $url = $this->getActualizacionEndpoint('cliente', $id_cliente);
