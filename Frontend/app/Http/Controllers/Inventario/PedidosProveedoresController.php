        ...
        public function index()
{
    ...
    if (!$response['success']) {
        return view('inventarioviews.pedidoproveedores.index',
                    compact('pedidos', 'proveedores', 'ingredientes'))
               ->with('error', $response['error'] ?? 'Error al cargar pedidos');
    }
        ...
        public function show(int $id)
{
    ...
    if (!$response['success']) {
        return Redirect::route('pedidoproveedores.index')
                       ->with('error', 'No se pudo cargar el detalle: ' . $response['error']);
    }
    ...
    return view('inventarioviews.pedidoproveedores.show', compact('pedido', 'proveedores'));
}
...
{
    $response = $this->service->entregarPedido($id);

    if ($response['success']) {
        return Redirect::route('pedidoproveedores.index')
                       ->with('success', 'Pedido #' . $id . ' marcado como entregado. Inventario actualizado.');
    }
