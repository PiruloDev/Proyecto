...
@push('styles')
    ...
    <style>
        ...
        /* Estilo para la tabla interna de la card */
        .mini-table {
            font-size: 0.85rem;
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-4 main-content">
            ...
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-4 pb-2 mb-4">
                <div>
                    ...
                    <p class="text-muted small mb-0">Control de composición de productos terminados.</p>
                </div>
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4" id="recetasGrid">
                @forelse($recetas as $receta)
                    <div class="col receta-item">
                        <div class="card h-100 shadow-sm receta-card">
                            <div class="card-body p-4 d-flex flex-column">
                                <div class="d-flex align-items-center justify-content-between mb-3">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 p-2" style="border-radius: 15px;">
                                            <li>
                                                <a class="dropdown-item rounded-3" href="{{ route('recetas.show', $receta['idProducto']) }}">
                                                    <i class="fas fa-eye me-2 text-primary"></i> Ver Detalle
                                                </a>
                                            </li>
                                            <li><hr class="dropdown-divider"></li>
                                            <li>
                                                <form method="POST" action="{{ route('recetas.destroy', $receta['idProducto']) }}">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="dropdown-item rounded-3 text-danger" onclick="return confirm('¿Eliminar esta receta?')">
                                ...
                                <p class="text-muted small mb-3">ID Producto: #{{ $receta['idProducto'] }}</p>

                                {{-- Vista previa de ingredientes --}}
                                <div class="mini-table p-2 mb-3">
                                    <table class="table table-sm table-borderless mb-0">
                                        ...
                                        <tbody>
                                            @foreach(array_slice($receta['detalles'], 0, 3) as $detalle)
                                                <tr>
                                                    <td><i class="bi bi-dot"></i> {{ $detalle['idIngrediente'] }}</td>
                                                    <td class="text-end fw-bold">{{ number_format($detalle['cantidadRequerida'], 2) }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                    @if(count($receta['detalles']) > 3)
                                        <div class="text-center pt-1">
                                            <small class="text-muted">+ {{ count($receta['detalles']) - 3 }} ingredientes más</small>
                                        </div>
                                    @endif
                                </div>
                ...
                @empty
                    <div class="col-12 text-center py-5">
                        <div class="p-5 glass-card rounded-4">
                            <i class="fas fa-utensils fa-3x mb-3 text-muted opacity-25"></i>
...
{{-- MODAL CREAR (Lógica unificada de la panadería) --}}
<div class="modal fade" id="crearModal" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content shadow-lg border-0">
            <div class="modal-header text-white border-0" style="background: var(--panaderia-marron-oscuro);">
                <h5 class="modal-title fw-bold"><i class="fas fa-plus-circle me-2"></i>Nueva Ficha Técnica</h5>
            ...
            <form method="POST" action="{{ route('recetas.store') }}" id="recetaForm">
                ...
                <div class="modal-body p-4">
                    <div class="mb-4">
                        <label class="form-label fw-bold small text-uppercase text-muted">Producto a Producir</label>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="fw-bold mb-0" style="color: var(--panaderia-marron-principal);">Composición de Ingredientes</h6>
                        <button type="button" id="add-detail-btn" class="btn btn-sm btn-panaderia px-3">
                            <i class="fas fa-plus"></i> Añadir
                        </button>
                    </div>
                    
                    <div id="detalles-container" class="rounded-4 p-2" style="background: #f8f9fa; border: 1px dashed #dee2e6;">
                        {{-- Se llena con JS --}}
                    </div>
                </div>
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-light rounded-3" data-bs-dismiss="modal">Cancelar</button>
                    <button type="submit" class="btn btn-panaderia px-4 fw-bold shadow">Guardar Receta</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('keyup', function() {
        ...
        let items = document.querySelectorAll('.receta-item');
    ...
    // Lógica para agregar filas de ingredientes en el modal
    ...
    addBtn.addEventListener('click', () => {
        ...
        div.className = 'row g-2 mb-2 align-items-center animate__animated animate__fadeIn';
        div.innerHTML = `
            <div class="col-md-7">
                <select name="detalles[${index}][idIngrediente]" class="form-select shadow-sm" required>
                    <option value="">Ingrediente...</option>
                    @foreach($ingredientesCatalogo as $ing)
                        <option value="{{ $ing->ID_INGREDIENTE }}">{{ $ing->NOMBRE_INGREDIENTE }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                <input type="number" step="0.001" name="detalles[${index}][cantidadRequerida]" class="form-control shadow-sm" placeholder="Cant." required>
            </div>
            <div class="col-md-1">
